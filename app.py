...
page = st.sidebar.radio("Go to", ["Home", "Provider", "Consumer", "Community Storage","Volunteer", "Dashboard","Help Desk"])

# Home Page
if page == "Home":
    st.markdown("<div class='main-title'>Welcome to Our Food Network</div>", unsafe_allow_html=True)
    ...
    st.image(image, caption="Discover Amazing Restaurants!", use_column_width=True)
...
# Community Storage Page
    ...
    buffer = BytesIO()
    qr.save(buffer)
    st.image(buffer.getvalue(), caption="Your Unique QR Code", use_column_width=False)
...
# MyDashboard
    ...
    st.markdown("<div class='main-title'>My Dashboard</div>", unsafe_allow_html=True)
